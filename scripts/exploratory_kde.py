...
def run_kde(input_path: str, out_dir: str, resolution: float, bandwidths: list):
    ...
    try:
        ...
        print(
            "pip install geopandas scikit-learn rasterio matplotlib shapely pandas numpy\n"
        )
        return
    ...
    gdf = gpd.GeoDataFrame(
        {"nome": names, "type": types}, geometry=points, crs="EPSG:4326"
    )
    ...
    print(
        f"Grid size: {len(x_grid)} cols x {len(y_grid)} rows = {len(grid_points)} cells."
    )
    ...
    for bw in bandwidths:
        ...
        cb.set_label(
            "Densidad Comercial (Locales / km²)", color="white", fontweight="bold"
        )
        cb.ax.yaxis.set_tick_params(colors="white")

        ax.tick_params(colors="white")
        ax.set_title(
            f"Londrina KDE Comercial — Bandwidth: {bw}m",
            color="white",
            fontsize=14,
            fontweight="bold",
            pad=15,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Londrina Commercial KDE Exploratory Analysis"
    )
    parser.add_argument(
        "--input", default="londrina_businesses.json", help="Path to input json"
    )
    parser.add_argument("--outdir", default="output", help="Output directory")
    parser.add_argument(
        "--px", type=float, default=100.0, help="Cell resolution size in meters"
    )
    parser.add_argument(
        "--bandwidths",
        nargs="+",
        ...
        help="KDE Bandwidths to process",
    )
